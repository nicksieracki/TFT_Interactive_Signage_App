...
@Component({
    ...
    template: `
        <div class="flex h-full w-full items-center justify-center bg-[#f7f8fa]">
            @if (safe_url(); as url) {
                <iframe [src]="url" class="h-full w-full border-0" title="Events"></iframe>
            } @else {
                <div
                ...
                >
                    <icon class="text-4xl text-[#038AED]">event</icon>
                    <p class="text-base font-medium">Events URL is not configured.</p>
                    <p class="text-sm text-gray-500">
                        Set events_url in the wayfinder_app metadata.
                    </p>
                </div>
            }
        </div>
...
export class EventsPage {
    ...
    private readonly _events_url = this._settings.signal<string>('events_url', '');

    readonly safe_url = computed<SafeResourceUrl | null>(() => {
        const url = this._events_url()?.trim();
